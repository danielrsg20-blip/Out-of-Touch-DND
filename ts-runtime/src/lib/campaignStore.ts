...
function saveToDisk(): void {
  try {
    ...
    writeFileSync(
      PERSIST_PATH,
      JSON.stringify([...campaigns.values()], null, 2),
      "utf-8",
    );
  } catch {
...
export function autoSaveSessionToCampaign(
  snapshot: SessionSnapshotForSave,
  campaignName?: string,
): void {
  ...
  const ownerId =
    existing?.owner_id ??
    snapshot.players.find((p) => p.user_id)?.user_id ??
    null;
