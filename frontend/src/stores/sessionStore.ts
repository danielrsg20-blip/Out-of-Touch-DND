...
function readActiveSession(): StoredSessionData | null {
  try {
    ...
    return {
      ...
      campaignId:
        typeof parsed.campaignId === "string" ? parsed.campaignId : null,
    };
  } catch {
    return null;
  }
...
function readBattlemapQualityMode(
  ...
  try {
    const raw = window.localStorage.getItem(
      battlemapQualityModeStorageKey(roomCode),
    );
...
function writeBattlemapQualityMode(
  roomCode: string | null | undefined,
    ...
    .on(
      ...
      async (payload) => {
        ...
        try {
          ...
          const latestSession = latestState.session as
          ...
          console.warn(
            "Failed to refresh session after realtime event.",
            error,
          );
        }
      },
...
async function syncStateWithPendingPolling(
  roomCode: string,
  source: "create" | "join",
): Promise<void> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (!isBattlemapPending(stateToSync)) {
      if (attempt > 1) {
        console.info(
          `Battlemap became ready after ${attempt} ${source} sync attempts.`,
        );
      }
      return;
    }
  ...
  console.warn(
    "Battlemap is still pending after polling window. It will update on the next session sync.",
  );
}
...
export const useSessionStore = create<SessionState>((set) => ({
  ...
  createSession: async (
    ...
    if (!shouldUseSupabaseSessions()) {
      ...
      if (
        ...
        throw new TypeError(
          "Unable to create session (invalid server response).",
        );
      }
      set({
        sessionId: null,
      ...
      void (async () => {
        try {
          ...
          console.error(
            "Failed to sync initial game state after session create:",
    ...
    try {
      data = await invokeEdgeFunction<Record<string, unknown>>(
        "session-actions",
        {
      ...
      supabaseCreateError =
        error instanceof Error
          ? error.message
          : "Supabase create_session failed.";
  ...
  joinSession: async (roomCode, playerName) => {
    ...
    if (!shouldUseSupabaseSessions()) {
      ...
      if (typeof data.player_id !== "string") {
        throw new TypeError(
          "Unable to join session (invalid server response).",
        );
      }
      const players = Array.isArray(session?.players)
        ? (session?.players as PlayerData[])
      ...
      void (async () => {
        try {
          ...
          console.error(
            "Failed to sync initial game state after session join:",
            error instanceof Error ? error.message : error,
          );
        }
      })();
    ...
    try {
      ...
      if (res.ok) {
        ...
        const campaigns = Array.isArray(data.campaigns)
          ? (data.campaigns as CampaignSlot[])
          : [];
        set({ campaigns });
      }
    } catch {
  ...
  resumeCampaign: async (campaignId, playerName) => {
    ...
    if (!campaignRes.ok || typeof campaignRes.data.error === "string") {
      throw new Error(
        typeof campaignRes.data.error === "string"
          ? campaignRes.data.error
          : "Campaign not found.",
      );
    }
    const campaign = campaignRes.data as {
      ...
      name: string;
      player_characters: Record<
        string,
        { char_id: string; name: string; class: string; level: number }
      >;
    };

    // 2. Create a fresh Supabase session
    const createData = await invokeEdgeFunction<Record<string, unknown>>(
      "session-actions",
      {
        action: "create_session",
        player_name: playerName,
    ...
    if (
      typeof createData.room_code !== "string" ||
      typeof createData.player_id !== "string"
    ) {
    ...
    const sessionId =
      typeof createData.session_id === "string" ? createData.session_id : null;

    // 3. Determine which character belongs to this user
    const userId = useAuthStore.getState().userId;
    const myCharEntry = userId
    ...
    const initData = await invokeEdgeFunction<Record<string, unknown>>(
      "session-actions",
      {
    ...
    const restoredState = initData.state as
      | {
    ...
    if (restoredState) {
      ...
      if (
        restoredState.characters &&
        Object.keys(restoredState.characters).length > 0
      ) {
        setCharacters(
          restoredState.characters as Record<
            string,
            import("../types").CharacterData
          >,
        );
      }
      ...
      if (Array.isArray(restoredState.narrative_history)) {
        for (const entry of restoredState.narrative_history as Array<{
          type?: string;
          content?: string;
          speaker?: string;
        }>) {
