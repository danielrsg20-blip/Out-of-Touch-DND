...
interface GameState {
  ...
  skillChallenge: SkillChallengeData | null;
  ...
  setSkillChallenge: (sc: SkillChallengeData | null) => void;
  setAoePreview: (aoe: AoePreview | null) => void;
  setWorldTime: (wt: WorldTime | null) => void;
...
function normalizeClassFeatures(
  value: unknown,
...
function normalizeCharacter(character: CharacterData): CharacterData {
  ...
  return {
    ...
    proficiency_bonus: toFiniteNumber(
    ...
    spellcasting_mode:
      raw.spellcasting_mode === "known" ||
      raw.spellcasting_mode === "prepared" ||
      raw.spellcasting_mode === "none"
...
export const useGameStore = create<GameState>((set) => ({
  ...
  addEntity: (entity) =>
    set((s) => {
      if (!s.map) return s;
      const entities = [
        ...s.map.entities.filter((e) => e.id !== entity.id),
        entity,
      ];
      return { map: { ...s.map, entities } };
    }),
  ...
  setCharacters: (characters) =>
    set({ characters: normalizeCharacters(characters) }),
  setCombat: (combat) => set({ combat }),

  addNarrative: (type, content, speaker) =>
    set((s) => ({
      narrative: [
        ...s.narrative,
        {
  ...
  setSkillChallenge: (sc) => set({ skillChallenge: sc }),
  setAoePreview: (aoe) => set({ aoePreview: aoe }),
  setWorldTime: (wt) => set({ worldTime: wt }),
  ...
  syncState: (state) =>
    set((current) => {
      ...
      const shouldReplaceCharacters = normalizedIncomingCharacters
        ? Object.keys(normalizedIncomingCharacters).length > 0 ||
          Object.keys(current.characters).length === 0
        : false;

      const finalCharacters = shouldReplaceCharacters
        ? normalizedIncomingCharacters!
        : current.characters;
      ...
      if (finalMap) {
        ...
        const missing = Object.entries(finalCharacters)
          ...
          .map(([charId, char]) => ({
            ...
            type: "pc" as const,
            sprite:
              typeof char.sprite_id === "string" && char.sprite_id.trim()
                ? char.sprite_id
                : `pc_${(char.race ?? "human")
                    .trim()
                    .toLowerCase()
                    .replaceAll(
                      /[\s_]+/g,
                      "-",
                    )}_${(char.class ?? "fighter").trim().toLowerCase()}`,
          }));
        if (missing.length > 0) {
          finalMap = {
            ...finalMap,
            entities: [...finalMap.entities, ...missing],
          };
        }
      }
      ...
      return {
        ...
        worldTime: state.world_time ?? current.worldTime,
      };
    }),
}));
