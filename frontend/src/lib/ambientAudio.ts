...
type BiomeTheme = 'forest' | 'dungeon' | 'tavern' | 'cave' | 'coastal' | 'mountain' | 'ruins' | 'city' | 'silence'
...
function loopNoise(gain: number, filterFreq: number, q = 1): AmbientLayer {
  ...
  return { stop: () => { try { src.stop(); src.disconnect() } catch { /* ignore */ } } }
}
...
function windGust(interval: [number, number]): AmbientLayer {
  ...
  return { stop: () => { alive = false; if (timeout) clearTimeout(timeout) } }
}
...
function mapLocationToBiome(location: string | undefined, biome: string | undefined): BiomeTheme {
  const loc = (location ?? '').toLowerCase()
  const bio = (biome ?? '').toLowerCase()

  if (loc === 'tavern') return 'tavern'
  if (loc === 'dungeon') return 'dungeon'
  if (loc === 'cave') return 'cave'
  if (loc === 'coastal') return 'coastal'
  if (loc === 'mountain') return 'mountain'
  if (loc === 'ruins') return 'ruins'
  if (loc === 'city_alley') return 'city'
  if (bio === 'underground') return 'cave'
  if (bio === 'urban') return 'city'
  if (bio === 'arctic') return 'mountain'
  if (loc === 'forest' || bio === 'temperate' || bio === 'tropical') return 'forest'
  return 'silence'
}

function stopAll() {
  for (const layer of activeLayers) {
    try { layer.stop() } catch { /* ignore */ }
  }
