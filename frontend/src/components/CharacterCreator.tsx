    ...
    try {
      ...
      try {
        payload = await invokeEdgeFunction<Record<string, unknown>>('dm-action', {
          action: 'get_spell_options', char_class: nextClass, level: 1, mock_mode: mockMode,
        })
      } catch {
        const res = await fetch(`${API_BASE}/api/spells/options/${encodeURIComponent(nextClass)}/1`)
        payload = await parseJsonBody(res)
        if (!res.ok) throw new Error(typeof payload.error === 'string' ? payload.error : 'Unable to load spell options.')
      }
