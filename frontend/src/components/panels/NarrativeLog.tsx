...
function parseDiceResult(
  content: string,
): { label: string; result: string } | null {
...
export default function NarrativeLog() {
  ...
  // Refresh relative timestamps every 60 seconds while timestamps are visible
  useEffect(() => {
  ...
  const handleExport = () => {
    const lines = narrative.map((e) => {
      const ts = new Date(e.timestamp).toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      });
      const prefix = e.speaker
        ? `${e.speaker}: `
        : `[${e.type.toUpperCase()}] `;
      return `[${ts}] ${prefix}${e.content}`;
    });
    const blob = new Blob([lines.join("\n")], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `adventure-log-${new Date().toISOString().slice(0, 10)}.txt`;
    a.click();
