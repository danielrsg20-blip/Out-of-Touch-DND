...
// Mix of warm ember colors: gold, amber, orange
const EMBER_COLORS = [
  "#e4a853",
  "#ff7a35",
  "#f0c060",
...
const PARTICLES = Array.from({ length: 38 }, (_, i) => {
  ...
  const size = 1.2 + Math.abs(Math.sin(i * 3.7)) * 3.2;
  const duration = 7 + Math.abs(Math.cos(i * 1.2)) * 12;
  const delay = -(Math.abs(Math.sin(i * 0.9 + 1)) * 11);
  const opacity = 0.18 + Math.abs(Math.sin(i * 2.1 + 0.5)) * 0.42;
  const xDrift = (i % 2 === 0 ? 1 : -1) * (2 + Math.abs(Math.sin(i * 1.7)) * 7);
  const yDrift = 22 + Math.abs(Math.cos(i * 2.3)) * 14;
  const color = EMBER_COLORS[i % EMBER_COLORS.length];
  // Larger particles get a halo glow
  const glowColor = size > 2.5 ? color : "#e4a853";
  const glowSpread = size > 2.5 ? Math.round(size * 2) : 3;
  return {
    left,
    top,
    size,
    ...
    >
        ...
        {PARTICLES.map((p) => (
          <motion.span
            ...
            style={{
              ...
              opacity: p.opacity,
              background: p.color,
              boxShadow: `0 0 ${p.glowSpread}px ${Math.ceil(p.glowSpread / 2)}px ${p.glowColor}88`,
            }}
            animate={{
              x: [0, p.xDrift, p.xDrift * 0.3, 0],
              y: [0, -p.yDrift * 0.5, -p.yDrift, -p.yDrift * 1.4],
              scale: [1, 1.2, 0.8, 0.3],
              opacity: [p.opacity, p.opacity * 1.15, p.opacity * 0.6, 0],
            }}
            transition={{
              duration: p.duration,
              delay: p.delay,
      ...
      >
        <Card
          ...
          style={{
            boxShadow:
              "0 8px 48px rgba(0,0,0,0.75), 0 0 0 1px rgba(228,168,83,0.07), inset 0 1px 0 rgba(228,168,83,0.12)",
          }}
        >
          ...
          <form className="w-full flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col gap-1.5">
              <label
                ...
                className="text-[0.75rem] uppercase tracking-[0.07em] text-[#a0a0b0] flex items-center gap-1.5"
              >
                Username
                {mode === "register" && username.length > 0 && (
                  <span
                    className={cn(
                      "text-[0.7rem] font-semibold normal-case tracking-normal",
                      usernameValid ? "text-[#2ecc71]" : "text-[#e74c3c]",
                    )}
                  >
              ...
              >
                ...
                {mode === "register" && password.length > 0 && (
                  <span
                    className={cn(
                      "text-[0.7rem] font-semibold normal-case tracking-normal",
                      passwordValid ? "text-[#2ecc71]" : "text-[#e74c3c]",
                    )}
                  >
                ...
                >
                  ...
                  {confirm.length > 0 && (
                    <span
                      className={cn(
                        "text-[0.7rem] font-semibold normal-case tracking-normal",
                        confirmValid ? "text-[#2ecc71]" : "text-[#e74c3c]",
                      )}
                    >
