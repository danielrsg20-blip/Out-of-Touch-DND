...
function uniqueStrings(values: string[]): string[] {
  return Array.from(
    new Set(values.filter((v) => typeof v === "string" && v.trim().length > 0)),
  );
}
...
function inferEnemySpriteIdByName(name: string): string {
  ...
  if (key.includes("wolf") || key.includes("boar") || key.includes("bat"))
    return "enemy_wolf";
...
function inferPropSpriteIdByName(name: string): string {
  const key = name.toLowerCase();
  if (key.includes("tree") || key.includes("bush") || key.includes("log"))
    return "prop_tree";
  if (
    key.includes("urn") ||
    key.includes("tomb") ||
    key.includes("bones") ||
    key.includes("brazier")
  )
    return "prop_urn";
  if (
    key.includes("stalagmite") ||
    key.includes("crystal") ||
    key.includes("mushroom")
  )
    return "prop_stalagmite";
...
export default function MapCanvas({
  ...
  const characterSheetCacheRef = useRef<
    Map<string, HTMLImageElement | "loading" | null>
  >(new Map());
  const monsterSheetCacheRef = useRef<
    Map<string, HTMLImageElement | "loading" | null>
  >(new Map());
  const spriteCacheRef = useRef<
    Map<string, HTMLImageElement | "loading" | null>
  >(new Map());
  const enemyMonsterVariantByEntityIdRef = useRef<Map<string, string>>(
    new Map(),
  );
  const enemyFrameKeysRef = useRef<Map<string, string[]>>(new Map());
  const artifactImageRef = useRef<{
    collisionMask: HTMLImageElement | null;
    costMap: HTMLImageElement | null;
  }>({
    collisionMask: null,
    costMap: null,
  });
  ...
  const prevEntityPositionsRef = useRef<Map<string, { x: number; y: number }>>(
    new Map(),
  );
  ...
  const aoePreview = useGameStore((s) => s.aoePreview);
  ...
  const [transformCopyStatus, setTransformCopyStatus] = useState<
    "idle" | "copied" | "failed"
  >("idle");
  ...
  const correctionModeEnabled = useMapEditorStore(
    (s) => s.correctionModeEnabled,
  );
  const setCorrectionModeEnabled = useMapEditorStore(
    (s) => s.setCorrectionModeEnabled,
  );
  ...
  const showCollisionMaskLayer = useMapEditorStore(
    (s) => s.showCollisionMaskLayer,
  );
  const showCostMapLayer = useMapEditorStore((s) => s.showCostMapLayer);
  const toggleCollisionMaskLayer = useMapEditorStore(
    (s) => s.toggleCollisionMaskLayer,
  );
  ...
  const setArtifactLayerOpacity = useMapEditorStore(
    (s) => s.setArtifactLayerOpacity,
  );
  ...
  const runtimeOverlayForRender = useMemo(() => {
    ...
    return {
      ...
      layers: runtimeOverlay.layers.filter(
        (layer) =>
  ...
  useEffect(() => {
    ...
    const inferredWidth = Math.max(
      1,
      Math.round(Number(traversalGrid?.width_cells ?? map.width ?? 1)),
    );
    const inferredHeight = Math.max(
      1,
      Math.round(Number(traversalGrid?.height_cells ?? map.height ?? 1)),
    );
    const inferredCellSize = Math.max(
      0.25,
      Number(
        traversalGrid?.cell_size_world ??
          map.traversal_grid?.cell_size_world ??
          5,
      ),
    );
  ...
  const localPlayerEntityIds = useMemo(() => {
    ...
    const playerCharacterId = players.find(
      (p) => p.id === playerId,
    )?.character_id;
  ...
  const isLocalPlayerEntity = useCallback(
  ...
  const resolveCharacterForEntity = useCallback(
    (entityId: string, entityName: string) => {
      ...
      const fromPlayerMembership = players.find(
        (player) => player.id === entityId,
      )?.character_id;
  ...
  const imageOpacity = Math.min(
    1,
    Math.max(0, mapMetadata?.image_opacity ?? 1),
  );
  const renderCellWidth = mapGridTransform.cellWidthPx;
  const renderCellHeight = mapGridTransform.cellHeightPx;
  const tokenBaseSizePx = Math.max(
    12,
    Math.min(renderCellWidth, renderCellHeight) * 0.7,
  );

  const applyTraversalGridToState = useCallback(
    (nextGrid: FrontendTraversalGrid) => {
  ...
  const applyCorrectionBrushAtCell = useCallback(
    (centerX: number, centerY: number) => {
      ...
      const nextCells = traversalGrid.cells.map((cell) => ({
        ...cell,
        movement_blocking_tags: Array.isArray(cell.movement_blocking_tags)
      ...
      for (let oy = 0; oy < brushSpan; oy += 1) {
        for (let ox = 0; ox < brushSpan; ox += 1) {
          ...
          if (activeTool === "paint_blocked") {
            ...
            cell.movement_blocking_tags = uniqueStrings([
              "blocked",
              "wall",
              ...cell.movement_blocking_tags,
            ]);
            ...
            cell.movement_blocking_tags = [];
            cell.tags = uniqueStrings(
              cell.tags
                .filter((tag) => tag !== "blocked" && tag !== "wall")
                .concat(["open_ground"]),
            );
            ...
            cell.movement_blocking_tags = [];
            const filteredTags = cell.tags.filter(
              (tag) =>
                tag !== "blocked" &&
                tag !== "wall" &&
                tag !== "open_ground" &&
  ...
  const getBrushTraversalCellsAtTraversalCell = useCallback(
    (centerX: number, centerY: number): Array<{ tx: number; ty: number }> => {
      if (!traversalGrid) {
        return [];
      }
  ...
  const traversalCellAtPointer = useCallback(
    (clientX: number, clientY: number): { tx: number; ty: number } | null => {
      ...
      const { gx, gy } = interaction.screenToGrid(clientX, clientY, rect, {
    ...
    [
      interaction,
      map,
      mapGridTransform.mapHeightPx,
      mapGridTransform.mapWidthPx,
      traversalGrid,
    ],
  );

  const paintAtPointerPosition = useCallback(
  ...
  const updateHoverCellFromPointer = useCallback(
    (clientX: number, clientY: number) => {
  ...
  const handleFetchPreviewArtifacts = useCallback(async () => {
    if (!map || !map.metadata?.image_url || !roomCode) {
      addNarrative(
        "system",
        "Cannot preview traversal artifacts without a map image and active room.",
      );
      return;
    }
    ...
    const mapBiome =
      typeof map.metadata.environment === "string"
        ? map.metadata.environment.trim().toLowerCase()
        : "";
    ...
    const widthFeet = Math.max(
      20,
      Math.round(preferredGridWidthCells * previewCellSizeWorld),
    );
    const heightFeet = Math.max(
      20,
      Math.round(preferredGridHeightCells * previewCellSizeWorld),
    );

    setPreviewBusy(true);
    try {
      const response = await callBackendApi(
        "/api/tools/import_battlemap_preview",
        {
          method: "POST",
          body: {
            ...
            scene_spec: {
              location,
              biome,
              encounter_type: sanitizeEncounterType(
                map.metadata.encounter_type,
              ),
              notable_features: Array.isArray(map.metadata.tactical_tags)
                ? map.metadata.tactical_tags.slice(0, 6)
                : [],
      ...
      const returnedGrid = response.data?.traversal_grid as
        | FrontendTraversalGrid
        | undefined;
      if (
        returnedGrid &&
        Array.isArray(returnedGrid.cells) &&
        returnedGrid.cells.length > 0
      ) {
        applyTraversalGridToState(returnedGrid);
      }

      const diagnostics = response.data?.diagnostics as
        | Record<string, unknown>
        | undefined;
      const artifacts = diagnostics?.preview_artifacts as
        | Record<string, unknown>
        | undefined;
      setPreviewArtifacts({
        collision_mask_png_base64:
          typeof artifacts?.collision_mask_png_base64 === "string"
            ? artifacts.collision_mask_png_base64
            : undefined,
        cost_map_png_base64:
          typeof artifacts?.cost_map_png_base64 === "string"
            ? artifacts.cost_map_png_base64
            : undefined,
      });

      addNarrative(
        "system",
        "Loaded traversal preview artifacts for correction mode.",
      );
      clearDirty();
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Unable to refresh preview artifacts.";
    ...
    applyTraversalGridToState,
    clearDirty,
    editorGridCellSizeWorld,
    editorGridHeightCells,
    editorGridWidthCells,
    map,
    roomCode,
    setPreviewArtifacts,
    setPreviewBusy,
  ...
  const handleApplyCorrections = useCallback(async () => {
    if (!map || !traversalGrid || !roomCode) {
      addNarrative(
        "system",
        "Cannot apply corrections without an active room, map, and traversal grid.",
      );
      return;
    }
    ...
    try {
      ...
      const gameState = response.data?.game_state as
        | Record<string, unknown>
        | undefined;
      const updatedMap = (gameState?.map ?? mapPatch) as MapData;
      setMap(updatedMap);

      const updatedTraversal = (updatedMap?.traversal_grid ??
        traversalGrid) as FrontendTraversalGrid;
      ...
      addNarrative(
        "system",
        "Applied traversal corrections to runtime session state.",
      );
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Unable to apply traversal corrections.";
  ...
  useEffect(() => {
    ...
    previewArtifacts.collisionMaskPngBase64,
    previewArtifacts.costMapPngBase64,
  ]);
  ...
  useEffect(() => {
    ...
    const localTokenEntity =
      map.entities.find((entity) => isLocalPlayerEntity(entity)) ?? null;
    ...
    const diagnostics = {
      ...
      traversal_cell_size_world: Number.isFinite(cellSizeWorld)
        ? cellSizeWorld
        : null,
      ...
      local_token_position: localTokenEntity
        ? { x: localTokenEntity.x, y: localTokenEntity.y }
        : null,
      local_token_visible: localTokenEntity
        ? Array.isArray(map.visible)
          ? map.visible.some(
              (v) => v.x === localTokenEntity.x && v.y === localTokenEntity.y,
            )
          : null
        : null,
    };
    ...
    mapMetadata?.image_width_px,
    mapMetadata?.image_height_px,
    mapMode,
    renderCellWidth,
    renderCellHeight,
    tokenBaseSizePx,
    ...
    const recoverMissingMap = async () => {
      for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        try {
          ...
          const stateToSync =
            (payload?.game_state as Record<string, unknown> | undefined) ??
            payload;
          ...
          if (!currentMap && payload?.battlemap_asset) {
            const merged = mergeBattlemapAssetIntoMap(
              currentMap,
  ...
  const getMonsterFrameKeyForEnemy = useCallback(
    (
      entityId: string,
      enemyName: string,
      explicitSpriteKey?: string,
    ): string | null => {
      ...
      if (explicitKey && explicitKey.toLowerCase() !== "default") {
        ...
        if (explicitRect) {
          ...
          const allFrames = getMonsterFrameKeysForBaseLabel(
            explicitRect.baseLabel,
          );
          frameCache.set(
            entityId,
            allFrames.length > 0 ? allFrames : [explicitRect.frameKey],
          );
      ...
      if (existing && resolveMonsterSpriteRect(existing)) {
        if (!frameCache.has(entityId)) {
          ...
          const allFrames = getMonsterFrameKeysForBaseLabel(
            existingRect.baseLabel,
          );
          frameCache.set(
            entityId,
            allFrames.length > 0 ? allFrames : [existing],
          );
        }
        return existing;
      }
      ...
      for (const candidate of candidates) {
        ...
        if (directRect) {
          ...
          const allFrames = getMonsterFrameKeysForBaseLabel(
            directRect.baseLabel,
          );
          frameCache.set(
            entityId,
            allFrames.length > 0 ? allFrames : [directRect.frameKey],
          );
  ...
  useEffect(() => {
    ...
    interaction.fitToView(map.width, map.height, rect.width, rect.height, {
      cellWidthPx: mapGridTransform.cellWidthPx,
      cellHeightPx: mapGridTransform.cellHeightPx,
    });
    fittedMapKeyRef.current = mapKey;
  }, [
    map,
    interaction,
    mapGridTransform.cellWidthPx,
    mapGridTransform.cellHeightPx,
  ]);
  ...
  const draw = useCallback(() => {
    ...
    if (correctionModeEnabled) {
      ...
      if (hoverCell) {
        ...
        if (traversalGrid) {
          const traversalCellWidthPx =
            mapGridTransform.mapWidthPx /
            Math.max(1, traversalGrid.width_cells);
          const traversalCellHeightPx =
            mapGridTransform.mapHeightPx /
            Math.max(1, traversalGrid.height_cells);
          const footprint = getBrushTraversalCellsAtTraversalCell(
            hoverCell.tx,
            hoverCell.ty,
          );
          ...
          for (const cell of footprint) {
            ...
            ctx.strokeRect(
              x + 0.5,
              y + 0.5,
              Math.max(1, traversalCellWidthPx - 1),
              Math.max(1, traversalCellHeightPx - 1),
            );
          }
          ctx.restore();
        } else {
          const rect = mapGridTransform.cellToPixelRect(
            hoverCell.tx,
            hoverCell.ty,
          );
          ctx.save();
    ...
    const visibleSet = new Set((map.visible || []).map((v) => `${v.x},${v.y}`));
    const hasVisibility = visibleSet.size > 0;

    if (mapMode !== MAP_MODE_AI) {
      console.error("[MapCanvas] Non-AI map mode reached gameplay renderer", {
        mapMode,
      });
      ctx.restore();
      return;
    ...
    if (runtimeOverlayForRender) {
      renderOverlayLayers(
        runtimeOverlayForRender,
        {
          ctx,
          mapBounds: {
            x: 0,
            y: 0,
            width: mapGridTransform.mapWidthPx,
            height: mapGridTransform.mapHeightPx,
          },
    ...
    // Fog of War overlay — drawn after vector props, before entity sprites.
    ...
    const layer2Props = map.entities.filter(
      (entity) => entity.type === "object",
    );
    const layer3Units = map.entities
    ...
    for (const entity of renderEntities) {
      ...
      const isDefeatedEnemy =
        entity.type === "enemy" && (characters[entity.id]?.hp ?? 1) <= 0;
      ...
      const monsterAllFrameKeys = monsterFrameKey
        ? (enemyFrameKeysRef.current.get(entity.id) ?? [])
        : [];
      const activeMonsterFrameKey =
        monsterAllFrameKeys.length > 1
          ? monsterAllFrameKeys[
              Math.floor(performance.now() / ANIM_FRAME_MS + entityIndex * 3) %
                monsterAllFrameKeys.length
            ]
          : monsterFrameKey;
      const monsterRect = activeMonsterFrameKey
        ? resolveMonsterSpriteRect(activeMonsterFrameKey)
        : null;
      ...
      const characterCell = characterFrameKey
        ? getCharacterSpriteCell(characterFrameKey)
        : null;
      const characterSheetUrl = characterFrameKey
        ? getCharacterSpritesheetUrl(characterFrameKey)
        : null;
      const characterSheetImage = characterSheetUrl
        ? characterSheetCacheRef.current.get(characterSheetUrl)
        : null;
      const isCharacterSheetSprite = Boolean(
        characterCell &&
        characterSheetImage &&
        characterSheetImage !== "loading",
      );
      ...
      if (isCharacterSheetSprite) {
        ...
        const sourceW =
          loadedSheet.naturalWidth / CHARACTER_SPRITESHEET_COLUMNS;
      ...
      const bobY =
        entity.type === "pc" && !isDefeatedEnemy
      ...
      const breathScale =
        !isDefeatedEnemy && entity.type !== "object"
      ...
      if (!spriteCacheRef.current.has(`__logged_${entity.id}`)) {
        spriteCacheRef.current.set(
          `__logged_${entity.id}`,
          "logged" as unknown as HTMLImageElement,
        );
        console.log(
          "[MapCanvas] entity",
          entity.id,
      ...
      if (entity.type !== "object") {
        ...
        ctx.ellipse(
          px,
          py,
          spriteDrawWidth * 0.38,
          renderCellHeight * 0.1,
          0,
          0,
          Math.PI * 2,
        );
      ...
      if (
        ...
        const sourceW =
          characterSheetImage.naturalWidth / CHARACTER_SPRITESHEET_COLUMNS;
        const sourceH =
          characterSheetImage.naturalHeight / CHARACTER_SPRITESHEET_ROWS;
        ...
        img.onerror = () => {
          console.error(
            "[MapCanvas] Failed to load character spritesheet:",
            characterSheetUrl,
          );
        ...
        img.onerror = () => {
          console.error(
            "[MapCanvas] Failed to load monster spritesheet:",
            MONSTER_SPRITESHEET_URL,
          );
        ...
        if (cached && cached !== "loading") {
          ...
          img.onerror = () => {
            console.error(
              "[MapCanvas] Failed to load sprite:",
              resolvedSpriteUrl,
            );
      ...
      if (entity.type !== "object") {
        const hpEntry = combat?.initiative_order.find(
          (e) => e.id === entity.id,
        );
        ...
        if (hp !== null && maxHp !== null && maxHp > 0) {
          ...
          const barColor =
            pct > 0.6 ? "#2ecc71" : pct > 0.3 ? "#f39c12" : "#e74c3c";
          ctx.save();
        ...
        if (conditions.length > 0) {
          ...
          const rowW =
            visibleConds.length * badgeW + (visibleConds.length - 1) * gap;
          ...
          visibleConds.forEach((cond, i) => {
            const info = CONDITION_INFO[cond.toLowerCase()] ?? {
              abbr: cond.slice(0, 3).toUpperCase(),
              color: "#95a5a6",
            };
    ...
    // AOE spell preview — drawn at hover cell position when targeting mode is active.
    if (aoePreview && hoverCellRef.current) {
      const { tx, ty } = hoverCellRef.current;
      const cellsRadius = aoePreview.size_ft / 5; // 1 cell = 5 ft
      const cellCx = tx * renderCellWidth + renderCellWidth / 2;
      const cellCy = ty * renderCellHeight + renderCellHeight / 2;
      ctx.save();
      ctx.globalAlpha = 0.35;
      ctx.fillStyle = "#e74c3c";
      ctx.beginPath();
      if (aoePreview.shape === "circle") {
        ctx.arc(cellCx, cellCy, cellsRadius * renderCellWidth, 0, Math.PI * 2);
        ctx.fill();
      } else if (aoePreview.shape === "cone") {
        const coneLen = cellsRadius * renderCellWidth;
        ctx.moveTo(cellCx, cellCy);
        ctx.arc(cellCx, cellCy, coneLen, -Math.PI / 6, Math.PI / 6);
        ctx.closePath();
        ctx.fill();
      } else if (aoePreview.shape === "square") {
        const halfSide = cellsRadius * renderCellWidth;
        ctx.fillRect(
          cellCx - halfSide / 2,
          cellCy - halfSide / 2,
          halfSide,
      ...
      } else if (aoePreview.shape === "line") {
        const lineLen = cellsRadius * renderCellWidth;
        ctx.fillRect(cellCx - 2, cellCy - lineLen, 4, lineLen);
      }
      ctx.globalAlpha = 0.7;
      ...
      if (aoePreview.shape === "circle") {
        ...
        const lineLen = cellsRadius * renderCellWidth;
        ctx.strokeRect(cellCx - 2, cellCy - lineLen, 4, lineLen);
      }
      ctx.stroke();
      ctx.restore();
    }
    ...
    imageUrl,
    imageOpacity,
    resolveCharacterForEntity,
    getMonsterFrameKeyForEnemy,
    targetingMode,
    runtimeOverlayForRender,
    showVectorLabels,
    showDmOnlyLabels,
    ...
    mapGridTransform,
    correctionModeEnabled,
    showCollisionMaskLayer,
    showCostMapLayer,
    artifactLayerOpacity,
    activeTool,
    getBrushTraversalCellsAtTraversalCell,
    isLocalPlayerEntity,
    aoePreview,
  ]);
  ...
  const handleCanvasPointerDown = useCallback(
    ...
    [
      activeTool,
      correctionModeEnabled,
      paintAtPointerPosition,
      updateHoverCellFromPointer,
    ],
  );
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      const clickedEntity = map.entities.find(
        (ent) => ent.x === gx && ent.y === gy,
      );
      if (clickedEntity) {
  ...
  const generationStatus =
    typeof mapMetadata?.generation_status === "string"
  ...
  if (!map) {
    ...
    return (
      <div className="map-placeholder">
        <p>
          No map loaded. The DM will generate one when the adventure begins.
        </p>
      </div>
    );
  }

  const hasBattlemapImage =
    typeof mapMetadata?.image_url === "string" &&
    mapMetadata.image_url.trim().length > 0;
  const isBattlemapPending =
    generationStatus === "pending" && !hasBattlemapImage;
      ...
      <div className="map-placeholder">
        <div className="map-placeholder-content">
          <p className="map-placeholder-text">
            Waiting for AI battlemap scene initialization...
          </p>
          <p style={{ fontSize: "0.85rem", margin: 0, opacity: 0.7 }}>
            Legacy terrain mode is disabled in gameplay scenes.
          </p>
        </div>
      </div>
    );
  ...
  const attributionLine = attributionRequired
    ? mapMetadata?.attribution_text?.trim() ||
      `Map art by ${mapMetadata?.author || "Unknown source"}`
    : "";
  const hasAttributionPanel = !!(
    mapMetadata?.author ||
    mapMetadata?.license_spdx ||
    ...
    >
      ...
      {correctionModeEnabled && (
        <div className="map-correction-panel" aria-live="polite">
          ...
          <label
            className="map-correction-cost-field"
            title="Set movement-cost paint value"
          >
            Movement value
            <input
              type="number"
              ...
              onChange={(event) =>
                setBrushMovementCost(Number(event.target.value))
              }
            />
          </label>
          <label
            className="map-correction-cost-field"
            title="Traversal cell size in world units (feet)"
          >
            Grid cell ft
            <input
              type="number"
          ...
          <div className="map-correction-grid-count-row">
            <label
              className="map-correction-cost-field"
              title="Traversal grid width (cells)"
            >
              Grid W
              <input
                type="number"
            ...
            <label
              className="map-correction-cost-field"
              title="Traversal grid height (cells)"
            >
              Grid H
              <input
                type="number"
          ...
          {activeTool !== "paint_cost" && (
            <div className="map-correction-tool-hint">
              Movement value applies to Paint cost.
            </div>
          )}
          <div
            className={`map-correction-dirty ${hasUnsavedChanges ? "is-dirty" : ""}`}
          >
          ...
          <label
            className="map-correction-opacity-field"
            title="Adjust preview artifact opacity"
          >
            Artifact opacity
            <input
              type="range"
              ...
              onChange={(event) =>
                setArtifactLayerOpacity(Number(event.target.value))
              }
            />
          </label>
      ...
      {hasAttributionPanel && (
        <div className="map-attribution-panel">
          ...
          {mapMetadata?.license_spdx && (
            <div>License: {mapMetadata.license_spdx}</div>
          )}
          {mapMetadata?.source_url && (
            <a
