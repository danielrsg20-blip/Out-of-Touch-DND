...
const ADVENTURE_HOOKS: AdventureHook[] = [
  {
    ...
    tone: "High Fantasy",
    premise:
      "Your party has been hired by a cloaked merchant to escort a sealed chest through the Thornwood. The coin is good, the questions are many, and the road ahead is anything but safe.",
  },
  {
    id: "ruins_aldenvoss",
    title: "Ruins of Aldenvoss",
    summary:
      "Ancient ruins surface beneath a town. Nobody who investigates comes back.",
    tone: "Mystery & Intrigue",
    premise:
      "Ancient ruins have been discovered beneath the town of Aldenvoss. Strange lights and unearthly sounds emerge every night. The mayor has posted a reward — but the last group that went in never returned.",
  },
  {
    id: "missing_village",
    title: "The Missing Village",
    summary:
      "An entire village vanished overnight. Tracks lead into dark forest.",
    tone: "Dark & Gritty",
    premise:
      "You arrive at the village of Millhaven to find it completely empty. No bodies, no sign of struggle — just abandoned meals, open doors, and a trail of strange footprints leading east into the forest.",
  },
  {
    id: "dark_compact",
    title: "The Dark Compact",
    summary:
      "A local lord has made deals with something sinister. His reach is long.",
    tone: "Mystery & Intrigue",
    premise:
      "Lord Varek of Stonebreach has grown obscenely wealthy over the past year. Rumours swirl of pacts with dark powers. Three investigators who looked into it have disappeared. You are next.",
  },
  {
    id: "cursed_caravan",
    ...
    tone: "High Fantasy",
    premise:
      "You find yourself on an unfamiliar road at dusk with no memory of how you got there. Ahead, a merchant caravan lies in ruins — burning wagons, scattered goods, and survivors who desperately need help.",
  },
  {
    id: "siege_of_redwall",
    title: "Siege of Redwall",
    summary:
      "Slip through enemy lines into a besieged fortress to retrieve something vital.",
    tone: "High Fantasy",
    premise:
      "The fortress town of Redwall has been under siege for three weeks. Inside its walls is something the enemy will kill to possess. Your party must breach the lines, reach the vault, and escape before the walls fall.",
  },
  {
    id: "sunken_temple",
    title: "The Sunken Temple",
    summary:
      "Treasure hunters keep going into a submerged temple. None come back up.",
    tone: "Dark & Gritty",
    premise:
      "Beneath the harbour of Port Maren lies the entrance to an ancient submerged temple. Three parties of treasure hunters have descended into its dark waters. None came back up.",
  },
  {
    id: "freeform",
    ...
    tone: "High Fantasy",
    premise:
      "The adventure begins where all great stories do — in a tavern. The world is wide, the road is open, and your fate is unwritten. Where do you want to go?",
  },
];
...
const PACING_OPTIONS = [
  { id: "slow-burn", label: "Slow Burn", icon: "🌘" },
  { id: "balanced", label: "Balanced", icon: "⚖️" },
  { id: "action-heavy", label: "Action Heavy", icon: "⚡" },
];
...
function FilledCampaignSlot({
  ...
  if (isConfirming) {
    ...
    slotStateClass =
      "flex-col items-start gap-2 bg-[rgba(231,76,60,0.04)] border-[rgba(231,76,60,0.6)]";
  } else {
    slotStateClass =
  ...
  return (
    ...
    >
      <div
        className="w-[22px] h-[22px] rounded-full bg-white/[0.06] border flex items-center justify-center text-[0.68rem] font-bold shrink-0"
        style={{ borderColor: color, color }}
      >
        {index + 1}
      </div>
      ...
      {!isConfirming && !isDeleting && (
        <>
          <div className="flex flex-col gap-0.5 flex-1 min-w-0">
            <span className="text-[0.88rem] font-semibold text-[#e0e0e0] truncate">
              {slot.name}
            </span>
              ...
              {slot.my_character ? (
                <>
                  ...
                  <span className="text-[#a0a0b0]">
                    {" "}
                    · {slot.my_character.class}
                  </span>
                  <span className="text-[#e4a853] font-semibold">
                    {" "}
                    Lv{slot.my_character.level}
                  </span>
                </>
              ) : (
            ...
            <div className="flex items-center gap-2.5 mt-0.5">
              <span className="flex items-center gap-1.5 text-[0.69rem] text-[#a0a0b0]">
                <span
                  ...
                  style={{
                    background: freshnessColor.bg,
                    boxShadow: freshnessColor.shadow,
                  }}
                />
                {dateStr}
              </span>
              <span className="text-[0.68rem] text-[#a0a0b0] bg-white/[0.05] px-1.5 py-0.5 rounded-lg border border-[#2a2a4a]">
                {slot.session_count} session
                {slot.session_count !== 1 ? "s" : ""}
              </span>
            </div>
          </div>
      ...
      {isConfirming && (
        <div className="flex flex-col gap-2 w-full">
          <span className="text-[0.82rem] text-[#e0e0e0]">
            Continue &ldquo;{slot.name}&rdquo;?
          </span>
      ...
      {isDeleting && (
        <div className="flex flex-col gap-2 w-full">
          <span className="text-[0.82rem] text-[#e0e0e0]">
            Delete &ldquo;{slot.name}&rdquo;? This cannot be undone.
          </span>
          <div className="flex gap-2 flex-wrap">
            <Button
              type="button"
...
export default function SessionLobby() {
  ...
  const [mode, setMode] = useState<
    "menu" | "create" | "campaign_setup" | "join"
  >("menu");
  ...
  const [selectedPacing, setSelectedPacing] = useState("balanced");
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to join session. Check the room code.",
      );
    }
  };
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse at 30% 40%, #1a2860 0%, #090d1f 55%, #0d0812 100%)",
      }}
    >
        ...
        {PARTICLES.map((p) => (
          <motion.span
            ...
            style={{
              ...
              opacity: p.opacity,
              background: "#e4a853",
              boxShadow: "0 0 5px 1px rgba(228,168,83,0.45)",
            }}
            animate={{
      ...
      >
        ...
        >
            ...
            <div className="flex items-center gap-2.5">
              <div
                ...
                style={{
                  ...
                  boxShadow: "0 0 0 2px rgba(228,168,83,0.3)",
                }}
                aria-hidden="true"
              >
              ...
              <div className="flex flex-col gap-0">
                <span className="text-[0.68rem] text-[#a0a0b0] leading-none">
                  Welcome back
                </span>
                <strong className="text-[0.88rem] text-[#e0e0e0] font-semibold leading-none mt-0.5">
                  {username}
                </strong>
              </div>
            </div>
          ...
          <div className="text-center mb-6">
            <h1 className="font-fantasy text-[1.7rem] font-bold text-[#e4a853] mb-1 tracking-[0.04em] gold-shimmer-text">
              Out of Touch DnD
            </h1>
            <p className="text-[#a0a0b0] text-[0.85rem] mb-4">
              LLM-Powered Campaign Engine
            </p>
            <div
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div>
                  <div className="flex items-center justify-between mb-2.5">
                    <h3 className="font-fantasy text-[0.65rem] uppercase tracking-[0.12em] text-[#a0a0b0] font-semibold">
                      Your Campaigns
                    </h3>
                    {!campaignsLoading && (
                          ...
                          if (!slot) {
                            ...
                            return (
                              <motion.div
                                ...
                                className={cn(
                                  ...
                                  atCap
                                    ...
                                    : "opacity-50 cursor-pointer hover:opacity-85",
                                )}
                                style={{ borderColor: color }}
                                onClick={() => {
                                  if (!atCap) setMode("create");
                                }}
                                role="button"
                                tabIndex={atCap ? -1 : 0}
                                onKeyDown={(e: React.KeyboardEvent) => {
                                  if (e.key === "Enter" && !atCap)
                                    setMode("create");
                                }}
                                aria-label={
                                  atCap
                              ...
                              >
                                ...
                                <span
                                  className="text-[1.1rem] leading-none ml-1"
                                  style={{ color }}
                                >
                                  {atCap ? "—" : "+"}
                                </span>
                                <span className="text-[0.78rem] text-[#a0a0b0]">
                                  {atCap ? "Limit reached" : "New Campaign"}
                                </span>
                              </motion.div>
                            );
                          }
            ...
            {mode === "create" && (
              ...
              >
                <h3
                  className="font-fantasy text-[0.82rem] font-semibold text-[#e4a853] uppercase tracking-widest pb-2 border-b border-[rgba(228,168,83,0.15)]"
                  style={{ textShadow: "0 0 10px rgba(228,168,83,0.2)" }}
                >
                  New Campaign
                </h3>
                <Input
                  ...
                  autoFocus
                  onKeyDown={(e: React.KeyboardEvent) =>
                    e.key === "Enter" && handleCampaignSetupNext()
                  }
                  className="bg-[rgba(26,26,62,0.85)] text-[#e0e0e0] placeholder:text-[#a0a0b0]/50"
                />
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2.5 max-h-80 overflow-y-auto pr-0.5">
                  {ADVENTURE_HOOKS.map((hook) => (
                    ...
                    >
                      <span className="font-fantasy text-[0.72rem] font-semibold text-white/90 leading-tight">
                        {hook.title}
                      </span>
                      <span className="text-[0.72rem] text-[#a0a0b0] leading-[1.35]">
                        {hook.summary}
                      </span>
                      <span className="text-[0.65rem] uppercase tracking-[0.08em] text-[#e4a853] opacity-75 mt-0.5">
                        {hook.tone}
                      </span>
                    </button>
                  ))}
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[0.75rem] text-[#a0a0b0]">
                    Or write your own premise:
                  </span>
                  <textarea
                    ...
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => {
                      ...
                      if (e.target.value.trim()) setSelectedHook(null);
                    }}
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[0.75rem] text-[#a0a0b0]">Pacing:</span>
                  <div className="flex flex-wrap gap-1.5">
          ...
          {roomCode && (
            ...
            >
              <div className="flex justify-between items-baseline mb-2">
                <span className="text-[0.82rem] text-[#e0e0e0]">
                  Room: <strong className="text-[#e4a853]">{roomCode}</strong>
                </span>
                <span className="text-[0.75rem] text-[#a0a0b0]">
              ...
              <ul className="list-none m-0 p-0 flex flex-col gap-1.5">
                {players.map((player) => (
                  <li
                    key={player.id}
                    className="flex items-center gap-2 text-[#e0e0e0] text-[0.83rem]"
                  >
                    <div
                      className="w-[26px] h-[26px] rounded-full flex items-center justify-center text-[0.72rem] font-bold text-[#a0a0b0] shrink-0 border border-[#2a2a4a]"
                      style={{
                        background: "linear-gradient(135deg, #2a3a6e, #0f3460)",
                      }}
                    >
