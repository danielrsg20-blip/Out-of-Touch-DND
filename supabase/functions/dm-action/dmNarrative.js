...
exports.generateDmNarrative = generateDmNarrative;
...
function summarizeSnapshotForPrompt(snapshot) {
    ...
    const combatSummary = combat?.is_active
        ...
        : 'inactive';
    const mapMeta = snapshot.map?.metadata ?? {};
...
function buildFallbackNarrative(playerName, content) {
...
async function generateDmNarrative(input) {
    ...
    try {
        if (provider === 'anthropic') {
            ...
            const response = await withTimeout(callApi('https://api.anthropic.com/v1/messages', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                const detail = typeof payload.error === 'object' && payload.error && typeof payload.error.message === 'string'
        ...
        if (provider === 'openai' || provider === 'groq') {
            ...
            const response = await withTimeout(callApi(baseUrl, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
